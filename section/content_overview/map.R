...
addLabel <- function(data) {
  data$label <- paste0(
    ...
    <tr><td>obitos:</td><td align="right">', data$deceased, '</td></tr>
    <tr><td>recuperados:</td><td align="right">', data$recovered, '</td></tr>
    <tr><td>ativos:</td><td align="right">', data$active, '</td></tr>
    </table>'
  )
...
map <- leaflet(addLabel(data_latest)) %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("confirmados","recuperados", "?bitos", "ativos")
  ) %>%
