gettabelafullData <- function(groupBy) {
  padding_left <- max(str_length(data_evolution_estados$value_new), na.rm = TRUE)
...
output$tabelafull <- renderDataTable({
  data       <- gettabelafullData("nome")
  columNames <- c(
    "Estados",
    "Casos Confirmados",
    "Novos casos Confirmados",
    "Casos Confirmados <br>(por 100k habitantes)",
    "Recuperados",
    "Novos Recuperados",
    "Óbitos",
    "Novos óbitos",
    "Casos Ativos",
    "Novos Casos Ativos",
    "Casos Ativos <br>(por 100k habitantes)")
  datatable(
    data,
    rownames  = FALSE,
    colnames  = columNames,
